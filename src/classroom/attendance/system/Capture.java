...
public class Capture extends javax.swing.JFrame {
    ...
    static {
        File file = new File("C:\\Users\\uwagb\\Downloads\\opencv_java249.dll");
        ...
        if (option == 0) {
            try {
                ...
                if (FacultySignUp.status){
                    FacultySignUp.show_field.setText(Filename);
                }
                if(StudentRegistrationView.status){
                    StudentRegistrationView.show_field.setText(Filename);
                }
            ...
            if(FacultySignUp.status){
                FacultySignUp.photo = baos.toByteArray();
            }
            if(StudentRegistrationView.status){
                StudentRegistrationView.photo = baos.toByteArray();
            }
                dispose();
            } catch (Exception e) {
                stopCam();
    ...
    private void CaptureImage(JLabel image) {
        try {
            ...
            if (getImage != null) {
                ...
                if(FacultySignUp.status){
                    ImageIcon imageico = new ImageIcon(new ImageIcon(Filename).getImage().getScaledInstance(FacultySignUp.show_label.getWidth(), FacultySignUp.show_label.getHeight(), Image.SCALE_DEFAULT));
                    FacultySignUp.show_label.setIcon(imageico);
                }
                if(StudentRegistrationView.status){
                    ImageIcon imageico = new ImageIcon(new ImageIcon(Filename).getImage().getScaledInstance(StudentRegistrationView.show_label.getWidth(), StudentRegistrationView.show_label.getHeight(), Image.SCALE_DEFAULT));
                    StudentRegistrationView.show_label.setIcon(imageico);
                }
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(rootPane, "Warning");
